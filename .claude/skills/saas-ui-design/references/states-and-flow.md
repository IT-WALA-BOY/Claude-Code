...
- **Skeleton screens wherever data is fetched.** Grey bars in the layout's shape. Nothing loaded faster; it just felt like it did. (Server-rendered pages that arrive complete need no skeleton; keep skeletons for client-fetched panels.)
...
**Motion that makes an app feel fast (from the Workflow build):**
- Optimistic updates: the tick, the counter or the moved card changes at once; the request follows; on failure it rolls back with a toast.
- Drag and drop that feels physical: the card lifts and tilts 2°, a dashed placeholder shows the drop spot, neighbours slide into place (FLIP), the card eases into its slot.
- Pages that open instantly: prerender on hover, then a 150ms fade of the content while the sidebar stays still.
- Undo in the toast for anything that removes an item from view (task done, item bought).
- Only `transform` and `opacity` animate; everything honours `prefers-reduced-motion`. Details: `build-notes.md` §4.
