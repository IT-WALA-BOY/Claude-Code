...
| `Calculated Value` | — | dashed `border/strong` on `bg/subtle`, value Body/Semibold, sub-line Meta muted |
...
| `Tag` | — | 0/6 padding, radius 4, hairline `border/strong`, Meta/Medium `text/secondary` (Shared, Refund, Reissue…) |
...
| `Stat Strip` | — | a row of Stats with vertical hairlines in one card, for secondary pages (not the flagship hero) |
...
| `Skeleton Row` | — | bars on `bg/skeleton`, radius 3, widths echoing the table columns |
...
| `Sidebar` | — | see dashboard-recipe §2 |
| `Top Bar` (optional) | — | 52h white bar, for products that must keep top navigation |
| `Impersonation Bar` (if roles) | — | warning-soft strip with the Return button |

## Overlays page
| `Menu Item` (4) | Type Default · Danger × State Default · Hover | 32h, radius 6, icon 16 + Body |
| `Menu` | — | `bg/surface`, `border/strong`, radius md, pad 4, Shadow/Menu, separators |
...
| `Drawer` | — | 760 × full height, right edge, head with actions (print, history, close), Stat Strip, table |
| `On scrim` example | — | Modal over `bg/scrim` |
