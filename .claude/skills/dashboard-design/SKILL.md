...
description: Design every screen of a dashboard, back-office, admin or SaaS product in Figma once its foundations exist. It covers the sitemap from the brief, a shared app shell (side panel and header), and proven page archetypes (overview, balances list, drawer over list, form with sticky summary, multi-section builder, master-detail, accounts, bills, reports, admin with permissions, sign-in). It also covers work-management screens (kanban board, Gantt timeline, week schedule, pipeline, 2x2 matrix, goal wizard, analytics), numbers that reconcile across screens, an accent budget, per-screen QA, flow sections on one page, and the handoff to code. Builds on /dashboard-design-system (tokens, components, flagship dashboard). Use when the user runs /dashboard-design or asks to design the remaining pages, CRUD flows, list/detail/form screens, reports or settings of a dashboard in Figma.
---

# /dashboard-design: the rest of the product, screen by screen

The Al Qafla Travel back-office proved this workflow. After the flagship dashboard (from `/dashboard-design-system`), ten more screens were built on the same page, from the same shell and components, with numbers that reconcile from screen to screen. The Workflow Dashboard (a freelancer's tasks, schedule, goals, outreach and money) repeated it for 14 screens and then built all of them in code; the archetypes 12 to 19 and the handoff step come from that build. This skill is how to repeat that for any product.
...
- `references/page-archetypes.md`: 19 screen recipes with measurements, content slots and pitfalls (1 to 11 finance back-office, 12 to 19 work management)
...
### Step 0b: Requirements from the file
If the user's file has reference images with notes, re-read every note before planning screens and list which note each screen answers. A note like "if we didn't discuss this feature, change the plan and add it" means a new screen (the Gantt timeline, the Analytics page and the real calendar all came from notes).
...
Run the audits (unbound paints, stale raw colours, empty icon instances, accent-touch count per screen, text overflow, em dashes). Take a final screenshot of every screen. Update `design-rules.md` in the project folder with the per-page intent table. Reply with the file link, the screen list with one sentence each, what reconciles across screens, and follow-ups (mobile views, empty/loading states, modals not yet drawn).

### Step 6: Handoff to code (when the user says "build it")
1. Dump every variable with its resolved value into the repo (`tokens.css`), and the data bible into a demo seed with dates relative to today.
2. Write a `code-standards.md` from saas-ui-design `references/build-notes.md` before the first line of code. It is the prompt that keeps AI-written code clean.
3. Build one screen at a time against its Figma screenshot (`get_screenshot` with base64 when asset URLs are blocked), and click through it in a real browser before the next.
...
- **Every draggable thing has a drawn drag state.** Tilted card (2°, scale 1.02, drag shadow) plus a dashed placeholder at the drop spot; Gantt bars show a black date tooltip ("Thu 24 to Sat 26 · 3 days") while moving.
- **No em dashes in any text layer.** Ranges read "9:30 to 11:00 PM", empty values read `0` or "Not set".
